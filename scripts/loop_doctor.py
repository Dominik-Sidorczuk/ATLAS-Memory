"""
Loop Doctor & Repository Health Diagnostic Tool (Repository Health Standard V23).
...
class LoopDoctor:
    ...
    def check_git_status(self) -> bool:
        ...
        try:
            ...
            # Auto-heal stale or truncated .git/index if detected
            index_path = self.root / ".git" / "index"
            try:
                if index_path.exists() and (index_path.stat().st_size < 32 or "mniejszy niż oczekiwano" in str(e) or "smaller than expected" in str(e)):
                    index_path.unlink(missing_ok=True)
                    subprocess.run(["git", "reset"], cwd=self.root, capture_output=True, check=True)
                    ...
                    untracked = [line for line in res_retry.stdout.splitlines() if line.strip()]
                    if not untracked:
                        self.passes.append("Git working tree is 100% clean (zero uncommitted changes, auto-healed index)")
                        return True
            except Exception:
                pass
