...
from loop_memory.active.prediction_error import ActiveSensingEngine, PredictionError
from loop_memory.active.shadow_worker import OmniRouteShadowWorker
...
from loop_memory.telemetry.cache_monitor import CacheHitMonitor
    ...
    async def process_telemetry_observation(
        ...
        if hasattr(self.orchestrator, "engine") and self.orchestrator.engine:
            async def _engine_triple_add(subject, predicate, object_, confidence, source, supersede):
                from loop_memory.models import EpistemicSource, MemoryRecord
