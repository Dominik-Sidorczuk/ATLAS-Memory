...
def numba_ttt_adapt_step(
    ...
    Zoptymalizowany pod kątem zerowych nadmiarowych alokacji (in-place error reuse).
    """
    # 1. Forward
    w_eff = w_base + w_ttt
    z = _numba_matmul_2d(x, w_eff)
    ...
    # 4. Wsteczny gradient dL/dW_ttt (reutylizacja macierzy diff bez zbędnej alokacji)
    factor = 2.0 / total_elements
    for i in range(n_rows):
        for j in range(n_cols):
            diff[i, j] *= factor

    # Bezkopiowy rzut transponowany (Numba radzi sobie ze strided view)
    grad_h = _numba_matmul_2d(diff, w_recon.T)

    grad_z = np.empty((grad_h.shape[0], grad_h.shape[1]), dtype=np.float64)
    for i in range(grad_h.shape[0]):
        for j in range(grad_h.shape[1]):
            grad_z[i, j] = grad_h[i, j] * (1.0 - h[i, j] * h[i, j])

    grad_w = _numba_matmul_2d(x.T, grad_z)
