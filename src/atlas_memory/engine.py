...
class HybridMemoryEngine:
    ...
    def __init__(
        ...
        self.ttt = ttt_layer or TTTLayer(input_dim=self.latent.state_dim)
        ...
        self.write_path = EpistemicWritePath(kv_store=self.kv, graph_store=self.graph)
    ...
    def create_default(
        ...
        ttt = TTTLayer(input_dim=latent.state_dim)
    ...
    def record_mental_transition(self, action: ActionPlan, session_id: str = "default") -> PredictedTransition:
        ...
        if self.ttt is not None and hasattr(self.ttt, "adapt_step"):
            try:
                vec = transition.predicted_state.vector
                target_dim = getattr(self.ttt, "input_dim", len(vec))
                if len(vec) != target_dim:
                    if len(vec) < target_dim:
                        vec = list(vec) + [0.0] * (target_dim - len(vec))
                    else:
                        vec = list(vec)[:target_dim]
                self.ttt.adapt_step(vec)
            except Exception as exc:
                logger.debug("TTT online adaptation error in record_mental_transition: %s", exc)
        return transition
