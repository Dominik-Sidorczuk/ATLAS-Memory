...
from atlas_memory.quantization import MIBQuantizer, QuantizationConfig, SIMDHamming
...
class QdrantVectorStore:
        ...
        if HAS_QDRANT:
            if location == ":memory:":
                ...
                try:
                    ...
                    lock_file = Path(location) / ".lock"
                    if lock_file.exists() and ("lock" in str(exc).lower() or "already accessed" in str(exc).lower()):
                        logger.warning("Detected stale Qdrant lock file at %s after SIGKILL/crash. Recovering...", lock_file)
                        try:
                            lock_file.unlink(missing_ok=True)
                            self.client = QdrantClient(path=location)
                            logger.info("Successfully recovered from stale Qdrant lock.")
                        except Exception as retry_exc:
                            logger.error("Failed to recover from Qdrant lock: %s", retry_exc)
                            raise
                    else:
                        raise
    ...
    def insert_sync(self, record: MemoryRecord, raw_text: Optional[str] = None) -> str:
        """Synchronously inserts a memory record into Qdrant or in-memory fallback storage."""
        ...
        if self.client is not None:
            point_id = (int(time.time() * 1_000_000) ^ (zlib.crc32(text.encode("utf-8")) & 0x7FFFFFFF)) % 0x7FFFFFFF
            self.client.upsert(
                collection_name=self.collection_name,
                points=[
                    qmodels.PointStruct(
    ...
    async def insert(self, record: MemoryRecord, raw_text: Optional[str] = None) -> str:
        """Asynchronously inserts a memory record under lock."""
        async with self._lock:
            return self.insert_sync(record, raw_text=raw_text)
