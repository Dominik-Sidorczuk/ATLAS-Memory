...
class RaBitQEngine:
    ...
    @staticmethod
    def _get_min_max_offsets(quantized: RaBitQResult) -> tuple[float, float]:
        """Zwraca spójne granice min/max dla kwantyzacji sub-bajtowej (z metadanych lub rozkładu)."""
        if quantized.offsets is not None and len(quantized.offsets) >= 2:
            return float(quantized.offsets[0]), float(quantized.offsets[1])
        sigma = 1.0 / np.sqrt(float(quantized.original_dim))
        c_factor = 3.0
        return -c_factor * sigma, c_factor * sigma
    ...
    def theoretical_error_bound(self, x: np.ndarray) -> float:
        ...
        bound = (||x||² / d) * (1 - 1 / 2^(2b)).
        """
        x_clean = np.nan_to_num(np.asarray(x, dtype=np.float32), nan=0.0, posinf=0.0, neginf=0.0)
        norm_sq = float(np.sum(x_clean ** 2))
    ...
    def quantize(
        ...
        if n_vectors == 0:
            if self.bits == 1:
                packed_data = np.empty((0, (self.dim + 7) // 8), dtype=np.uint8)
            elif self.bits == 4:
                packed_data = np.empty((0, (self.dim + 1) // 2), dtype=np.uint8)
            else:
                packed_data = np.empty((0, self.dim), dtype=np.uint8)
            return RaBitQResult(
                ...
                exact_vectors=np.empty((0, self.dim), dtype=np.float32) if store_exact else None,
            )
    ...
    def reconstruct(self, quantized: RaBitQResult) -> np.ndarray:
        ...
        min_val, max_val = self._get_min_max_offsets(quantized)
