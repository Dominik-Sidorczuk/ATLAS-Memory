...
class SalienceDecayEngine:
    ...
    def __init__(
        ...
        self._inv_log_norm = 1.0 / self._log_norm
    ...
    def calculate_salience(
        ...
        sim_part = self.w_sim * max(0.0, min(1.0, similarity_score))  # clamp01 scope: nie-confidence (waga/penalty)

        # 2. Składnik wewnętrznej ważności I(f)
        imp_part = self.w_imp * max(0.0, min(1.0, record.importance_score))  # clamp01 scope: nie-confidence (waga/penalty)
        ...
        freq_normalized = min(1.0, math.log1p(record.access_count) * self._inv_log_norm)  # clamp01 scope: nie-confidence (waga/penalty)
    ...
    def calculate_salience_batch(
        ...
        Używa zoptymalizowanych operacji wektorowych NumPy (np.exp, np.log1p, np.clip)
        z precyzją float64 dla znaczników czasu i szybką ekstrakcją iteratorów.
        """
        n = len(records)
        if n == 0:
        ...
        if similarity_scores is None:
            ...
            sims = np.asarray(similarity_scores, dtype=np.float32)
        sim_part = self.w_sim * np.clip(sims, 0.0, 1.0)

        importances = np.fromiter((r.importance_score for r in records), dtype=np.float32, count=n)
        imp_part = self.w_imp * np.clip(importances, 0.0, 1.0)

        # Użyj float64 dla timestampów zapobiegając utracie precyzji w epoch seconds
        timestamps = np.fromiter((r.timestamp for r in records), dtype=np.float64, count=n)
        dts = np.maximum(0.0, now - timestamps)
        rec_decay = np.exp(-self.decay_lambda * dts, dtype=np.float64)
        rec_part = (self.w_rec * rec_decay).astype(np.float32)

        access_counts = np.fromiter((r.access_count for r in records), dtype=np.float32, count=n)
        freq_normalized = np.minimum(1.0, np.log1p(access_counts) * self._inv_log_norm)
        freq_part = self.w_freq * freq_normalized
