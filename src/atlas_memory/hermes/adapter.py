...
class HermesMemoryAdapter:
    ...
    def __init__(
        ...
        omniroute_api_base: Optional[str] = None,
    ):
        self.orchestrator = orchestrator or MemoryOrchestrator()
        self.omniroute_api_base = omniroute_api_base or os.environ.get("ATLAS_OMNIRUTE_URL", "http://localhost:20128/v1")
        self.shadow_worker = shadow_worker or OmniRouteShadowWorker(
            orchestrator=self.orchestrator,
            api_base=self.omniroute_api_base,
        )
    ...
    def create_default(
        ...
        api_base = omniroute_api_base or os.environ.get("ATLAS_OMNIRUTE_URL", "http://localhost:20128/v1")
        model = omniroute_model or os.environ.get("ATLAS_MODEL", "qwen-mini")
    ...
    async def orchestrated_ingest(
        ...
        if hasattr(self.orchestrator, "engine") and self.orchestrator.engine is not None:
            if hasattr(self.orchestrator.engine, "record_mental_transition"):
                try:
                    from atlas_memory.models import ActionPlan
                    action = ActionPlan(name="sync_turn", parameters={"session_id": session_id})
                    self.orchestrator.engine.record_mental_transition(action, session_id=session_id)
                except Exception as exc:
                    logger.debug("Adapter orchestrated_ingest transition error: %s", exc)
    ...
    async def sync_turn(
        ...
        """Synchronizuje turę rozmowy i rejestruje w buforze trajektorii."""
        await self.orchestrated_ingest(user_msg, agent_response, session_id=session_id)
        return {"status": "ok", "synced": True, "session_id": session_id}
    ...
    async def commit_observation(
        ...
        session_id: str = HERMES_DEFAULT_SESSION,
    ) -> Dict[str, Any]:
        """Rejestruje obserwację w silniku pamięci oraz buforze trajektorii L1."""
        if hasattr(self.orchestrator, "engine") and self.orchestrator.engine is not None:
            from atlas_memory.models import ActionPlan, EpistemicSource, MemoryRecord
            record = MemoryRecord(
                subject=subject,
                predicate=predicate,
                object=str(obj),
            ...
            await self.orchestrator.engine.commit_observation(record)
            if hasattr(self.orchestrator.engine, "record_mental_transition"):
                try:
                    action = ActionPlan(name=f"observe:{predicate}", parameters={"subject": subject, "object": str(obj)})
                    self.orchestrator.engine.record_mental_transition(action, session_id=session_id)
                except Exception as exc:
                    logger.debug("Adapter commit_observation transition error: %s", exc)
            return {"status": "ok", "subject": subject}
        return {"status": "error", "error": "no_engine"}
