...
class EnergyModule:
    ...
    def register_prediction_error(self, state: str, action: str, discrepancy_score: float) -> None:
        ...
        key = f"{state}::{action}"
        self._error_penalties[key] = max(0.0, min(1.0, float(discrepancy_score)))  # clamp01 scope: nie-confidence (waga/penalty)
    ...
    async def intrinsic_cost(
        ...
        if current_confidence is not None:
            conf = clamp_conf(current_confidence)
            if base_cost > 0.4:
        ...
        return round(float(max(0.0, min(1.0, cost))), 6)  # clamp01 scope: nie-confidence (waga/penalty)
    ...
    async def trainable_critic(
        ...
        critical_ratio = min(1.0, max(0.0, critical_conf_sum / max(1e-5, total_conf_sum))) if total_conf_sum > 0 else 0.0  # clamp01 scope: nie-confidence (waga/penalty)

        raw_risk = (0.5 * critical_ratio) + (0.3 * branching_factor) + (0.2 * min(1.0, critical_conf_sum / 3.0))
        if not math.isfinite(raw_risk):
            raw_risk = 0.0

        return round(float(max(0.0, min(1.0, raw_risk))), 6)  # clamp01 scope: nie-confidence (waga/penalty)
