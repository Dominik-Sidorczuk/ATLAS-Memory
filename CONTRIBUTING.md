...
Every report containing quantitative data (latencies, token usage, compression) **must** include the raw JSON artifacts alongside the PR (e.g. under `tests/benchmark/` output or attached to the PR description). Reports without source data are treated as architectural targets, not empirical facts. Maintainers keep long-term regression baselines in a private `docs/baselines/` store.
