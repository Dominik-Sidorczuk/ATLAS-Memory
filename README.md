...
[![Tests: Passing (Semantic Gate)](https://img.shields.io/badge/tests-Passing%20(Semantic%20Gate)-success)](./tests)
...
|---|:---:|---|
| 🚪 **Retrieval Policy Gate** | **5.97 μs P50** | **95% noise reduction**; screens out chit-chat queries with **0 injected recall tokens**. |
| 🎒 **Epistemic Knapsack Packing** | **≤1500 tok ceiling** | **91.4% prompt token savings**; dynamic 0/1 Knapsack DP maximizes information density. |
...
| 🔒 **Cryptographic State Ledger** | **8,550+ SHA-256 blocks** | Tamper-evident Merkle hash-chain (prev_hash → entry_hash) guarantees immutable history. |
...
* **How It Works:** Memory facts are scored on two axes: **Veracity Score** (USER_EXPLICIT (1.0) > TOOL_OBSERVED (0.85) > AGENT_INFERENCE (0.50)) and **Token Cost**. ATLAS runs a dynamic programming 0/1 Knapsack algorithm to maximize total information density (I = score/tokens), guaranteeing that the injected context strictly respects the ≤1500 token budget ceiling.
...
* **How It Works:** ATLAS maintains a topological property graph in Kùzu DB. When the agent contemplates a destructive action, `atlas_what_if` traverses multi-hop dependency edges (e.g., `Obsidian_MCP:27124` → `Reporting_Pipeline` → `Daily_Master_Report`). If a path contains a Critical Point of Failure (**CPoF**), it raises an immediate `CRITICAL` risk alarm and hard-blocks execution until authorized by a human gate.
...
* **How It Works:** Every write to `state_variables` appends a block to `state_audit_log` where H_i = SHA-256(H_{i-1} || key || value || timestamp || reason). Any retroactive modification of past records invalidates the mathematical hash chain across all subsequent blocks.
...
|:---|:---|:---|
| 🧠 **Retrieval Policy Gate** | `orchestrator.py` | **5.97 μs P50** decision latency. Prevents memory retrieval on non-factual turns, achieving **95% noise reduction**. |
| 📦 **Epistemic Knapsack Packing** | `orchestrator.py` | Strict **≤1500 token budget** enforcement. Delivers **91.4% prompt token savings** using density-weighted knapsack optimization. |
...
| 05 | **Knapsack Token Packing (50 recs)**| **0.621 ms** *(621 μs)* | < 100 ms | Pure Mnemosyne: unbudgeted prompt dump (>15k tok) | 📦 **Strict ≤1500 tok** budget with 0/1 density opt |
...
| 11 | **Sleep Baking (20 trajs → SOP)**| **0.065 ms** *(65 μs)* | < 500 ms | LLM consolidation: 2,000–5,000 ms + token cost | 💤 **Deterministic distillation at 0 tokens** |
...
Communication between the two environments occurs over an ultra-low latency Unix Domain Socket (`~/.hermes/atlas.sock`) in <15 μs.
...
# 3. Run the full unit & integration test suite (Semantic Quality Gate)
pixi run test
...
* When Hermes executes the synchronous `prefetch()` hook, the result is already resolved in memory—achieving **<0.01 ms turn latency**.
...
* Rather than dumping raw database rows, `AtlasMemoryProvider` parses incoming Mnemosyne triples and assigns strict veracity weights:
  **Veracity:** USER_EXPLICIT (1.0) > TOOL_OBSERVED (0.85) > AGENT_INFERENCE (0.50)
* The 0/1 Knapsack algorithm packs facts to maximize total veracity under the strict ≤1500 token budget.

---


## 🛠️ First-Class Hermes Tool Calling (10 Native Tools)

ATLAS exposes 10 first-class JSON-RPC 2.0 tools directly to LLM agents over `~/.hermes/atlas.sock`:

| Tool | Parameters | Architectural Function & Response Shape |
|---|---|---|
| **`atlas_recall`** | `query: str`, `session_id: str = "hermes_default"` | Semantic retrieval over Kùzu graph, Qdrant vectors, and verified KV with <3 ms latency.<br>`{"status": "ok", "records": [...], "count": int}` |
...
| **`atlas_get`** | `key: str` | Fast O(1) state variable retrieval directly from Verified SQLite KV Store.<br>`{"status": "ok", "key": str, "found": bool, "state": dict}` |
| **`atlas_delete`** | `key: str`, `reason: str = "agent_delete"` | Atomic state variable deletion with SHA-256 audit log tracking.<br>`{"status": "ok", "key": str, "deleted": bool, "found": bool}` |
| **`atlas_task_feedback`** | `key: str`, `task_success: bool`, `delta: float = 0.05` | Closed-loop task feedback adapting record confidence and L0-TTT online weights.<br>`{"status": "ok", "key": str, "confidence": float, "task_success": bool}` |
| **`atlas_verify_chain`** | `heal: bool = false`, `deep: bool = false` | Cryptographic integrity verification and auto-healing of the SHA-256 audit hash-chain.<br>`{"status": "ok", "valid": bool, "checked_entries": int}` |
| **`atlas_sleep_sync`** | `skills_dir: str = None` | Forces immediate L3 sleep consolidation, Salience GC, and SOP skill baking.<br>`{"status": "ok", "consolidated_records": int, "pruned_stale_facts": int, "baked_sops_count": int, "baked_sops": list}` |

---

## 🎓 Hermes Skills Integration & Skill Interpretation

ATLAS integrates seamlessly with the Hermes Agent Skill Architecture across two distinct modalities:

### 1. The Operator Skill (`skills/atlas-cognitive-sidecar/SKILL.md`)
During plugin installation (`scripts/install-hermes-plugin.sh`), the canonical skill manifest is deployed to `~/.hermes/skills/memory/atlas-cognitive-sidecar/`.
* **Interpretation by Hermes:** Hermes continuously monitors `~/.hermes/skills/`. It parses the YAML frontmatter:
  ```yaml
  ---
  name: atlas-cognitive-sidecar
  description: Comprehensive operational rules for ATLAS Cognitive Memory Sidecar.
  trigger: Use when storing durable facts, preferences, running what-if simulations or active sensing.
  ---
  ```
* **Cognitive Guidance:** When user intent matches the trigger, Hermes injects the skill into the agent context, guiding the LLM on:
  - When to call `atlas_remember` (e.g. upon user explicit rules or state changes).
  - When to execute counterfactual `atlas_what_if` simulations before executing risky commands.
  - How to interpret `atlas_active_sensing` discrepancy severity (`CRITICAL`, `MODERATE`, `LOW`).

### 2. Autonomous Procedural Skill Baking (L3 Offline Sleep Consolidation)
ATLAS does not just follow static skills—it autonomically synthesizes and compiles new native Hermes skills from runtime experience:
1. **Trajectory Mining (`SleepBaker` in `src/atlas_memory/l3_procedural/sleep_baker.py`):** In offline sleep cycles (`_handle_trigger_sleep_consolidation`), ATLAS analyzes recorded multi-turn trajectories, clustering repeatable tool sequences that meet frequency ($\ge 2$) and success rate ($\ge 70\%$) thresholds into Standard Operating Procedures (`StandardProcedure` / SOP).
2. **Deterministic AST Security Scanning (`ASTSafetyScanner`):** Every synthesized procedure undergoes static AST inspection. Forbidden primitives (`eval`, `exec`, `__import__`, `subprocess.*`, `os.system`, dangerous write modes `w`, `a`) are strictly rejected with `SafetyViolationError`.
3. **Artifact Compilation (`skill_compiler.py`):** Verified SOPs are compiled into native Hermes skill bundles under `~/.hermes/skills/baked-skills/<procedure_id>/`:
   - `SKILL.md`: Standard Hermes markdown guide with YAML frontmatter, parameter schema, and step-by-step instructions.
   - `scripts/handler.py`: Fully executable Python class exposing sequential `step_N()` methods and `run()` wrapper.
4. **Instant Discovery:** Hermes automatically discovers and indexes the new skill in `~/.hermes/skills/`, making autonomous workflows directly invokable in subsequent sessions.

---

## 👤 Human-in-the-Loop & Operator Manual (Actions Outside Automation)

While ATLAS automates memory management, conflict arbitration, and skill synthesis, certain critical infrastructure tasks **cannot and must not** be executed automatically without human operator intervention:

| Task / Responsibility | Why it Cannot be Done via Script | Required Operator Action |
|---|---|---|
| **Host Tooling Setup** | Requires root/curl privileges on host OS | Install Pixi (`curl -fsSL https://pixi.sh/install.sh \| bash`) and Python 3.11+ before cloning. |
| **Hermes Agent Runtime** | User's personal Hermes installation | Ensure Hermes Agent is installed (`~/.hermes/venv/`) and configured in PATH. |
| **LLM Inference Credentials** | Security / Secret boundary | Configure your LLM API keys (`ANTHROPIC_API_KEY`, `OPENAI_API_KEY`) in `~/.hermes/.env` or `~/.hermes/config.yaml`. *(Note: ATLAS memory, vectors, and graph run 100% locally and require zero API keys)*. |
| **Manual Plugin Activation** | When running headless or without `hermes` CLI in PATH | Add `memory_provider: atlas` under `memory:` in `~/.hermes/config.yaml`. |
| **Production Systemd Service** | Requires system service supervisor setup | Create user systemd service (see snippet below) for persistent server reboots. |
| **Destructive Actions (Human Gate)** | AGENTS.md Hard Constraint | Deleting the knowledge base, database drops, and remote `git push` require explicit human authorization. |

### 🔧 Production Systemd User Service Setup (Optional)
If running on a headless Linux server where ATLAS should automatically start on boot:

```ini
# ~/.config/systemd/user/atlas-daemon.service
[Unit]
Description=ATLAS Cognitive Memory Daemon
After=network.target

[Service]
Type=simple
WorkingDirectory=%h/Hermes Agent/LOOP/Memory
ExecStart=%h/.pixi/bin/pixi run python scripts/atlas_daemon_launcher.py
Restart=always
RestartSec=5
StandardOutput=append:%h/.hermes/atlas/daemon.log
StandardError=append:%h/.hermes/atlas/daemon.log

[Install]
WantedBy=default.target
```

Enable and activate via systemctl:
```bash
systemctl --user daemon-reload
systemctl --user enable --now atlas-daemon.service
systemctl --user status atlas-daemon.service
```
...
├── 📄 LICENSE                    # MIT License
│
├── 📂 docs/                      # Visual blueprints and safety documentation
│   ├── 📂 assets/                # Diagrams and graphics (architecture, logo)
│   └── 📄 safety.md              # Safety policy and threat model
│
...
│   ├── cognitive/                # L1 Cognitive Domain Core (ConflictArbiter, PredictiveCoder, RetrievalPipeline, HotBufferPolicy, EpistemicWritePath)
...
│
└── 📂 tests/                     # Verified Test Suite (100% PASS, Semantic Quality Gate)
...
```bash
pixi run test         # Run the full test suite (~60 seconds)
